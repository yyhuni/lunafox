...
func TestGeneratedExecutionKeepsPortScanNamespacesTypedAndSeparate(t *testing.T) {
	...
	if configType.NumField() != 2 || configType.Field(0).Name != "NaabuActive" || configType.Field(0).Type != reflect.TypeOf(NaabuActiveConfig{}) || configType.Field(1).Name != "NaabuPassive" || configType.Field(1).Type != reflect.TypeOf(NaabuPassiveConfig{}) {
		t.Fatalf("Config = %v, want only typed Naabu section values", configType)
	}
	if reflect.TypeOf(PlatformResources{}).NumField() != 0 {
		t.Fatal("port scan must not expose undeclared platform resources")
	}
