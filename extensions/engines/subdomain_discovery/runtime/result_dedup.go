...
const (
	defaultResultDedupChunkByteBudget int64 = 8 * 1024 * 1024
	defaultResultDedupMergeFanIn            = 8
	resultDedupRecordOverhead         int64 = 64
)

// resultDedupOptions configures workspace-local external sorting for canonical
// subdomain results. Zero chunk and fan-in values select fixed runtime defaults.
...
// resultDedupStager stores byte-bounded sorted chunks below one private
// temporary directory in the execution workspace.
type resultDedupStager struct {
	tempDir         string
	chunkByteBudget int64
	mergeFanIn      int

	chunk       []storedResultDedupRecord
	chunkBytes  int64
	chunkCount  int
	nextOrdinal uint64
	inputCount  int
...
type storedResultDedupRecord struct {
	key     string
	payload []byte
	ordinal uint64
}
...
func newResultDedupStager(options resultDedupOptions) (*resultDedupStager, error) {
	...
	if chunkByteBudget == 0 {
		chunkByteBudget = defaultResultDedupChunkByteBudget
	}
	if chunkByteBudget < 0 {
	...
	if mergeFanIn == 0 {
		mergeFanIn = defaultResultDedupMergeFanIn
	}
	if mergeFanIn < 2 {
...
// add assigns source ordinals in parser order. stream retains the greatest
// ordinal so later valid scanner output is the complete observation for a key.
func (s *resultDedupStager) add(ctx context.Context, canonicalKey string, payload []byte) error {
	...
	recordBytes := estimateResultDedupRecordBytes(canonicalKey, storedPayload)
	...
	s.chunk = append(s.chunk, storedResultDedupRecord{
		key:     canonicalKey,
...
// stream globally deduplicates staged records in ascending canonical-key order.
// It removes temporary files before returning, including on cancellation or an
// emit failure.
func (s *resultDedupStager) stream(ctx context.Context, emit func(resultDedupRecord) error) (stats resultDedupStats, err error) {
	...
	var winner storedResultDedupRecord
	hasWinner := false
	err = mergeResultDedupRecords(ctx, finalPaths, func(record storedResultDedupRecord) error {
		if !hasWinner {
			winner = record
			hasWinner = true
			return nil
...
// close removes the private temporary directory. A failed removal can be
// retried after the underlying condition heals.
...
func writeResultDedupChunk(ctx context.Context, path string, records []storedResultDedupRecord) (err error) {
	...
	for _, record := range records {
		...
		if err := writeStoredResultDedupRecord(writer, record); err != nil {
...
func mergeResultDedupRecordsToFile(ctx context.Context, paths []string, outputPath string, observeOpenReaders func(int), observeLoadedPayloads func(int)) (err error) {
	...
	writer := bufio.NewWriter(file)
	err = mergeResultDedupRecords(ctx, paths, func(record storedResultDedupRecord) error {
		if err := writeStoredResultDedupRecord(writer, record); err != nil {
...
func mergeResultDedupRecords(ctx context.Context, paths []string, emit func(storedResultDedupRecord) error, observeOpenReaders func(int), observeLoadedPayloads func(int)) (err error) {
	if len(paths) == 0 {
		return nil
	}
	...
	for records.Len() > 0 {
		...
		if err := emitStoredResultDedupRecord(&readers[item.readerIndex], item.header, emit, observeLoadedPayloads); err != nil {
			return err
		}
		if err := resultDedupContextError(ctx); err != nil {
			return err
...
func emitStoredResultDedupRecord(reader *resultDedupChunkReader, header storedResultDedupRecordHeader, emit func(storedResultDedupRecord) error, observeLoadedPayloads func(int)) error {
...
func openResultDedupChunkReaders(paths []string) ([]resultDedupChunkReader, error) {
	...
	for _, path := range paths {
		...
		if err != nil {
			openErr := fmt.Errorf("open result deduplication merge input: %w", err)
			for _, reader := range readers {
				if closeErr := reader.file.Close(); closeErr != nil {
					openErr = errors.Join(openErr, fmt.Errorf("close result deduplication merge input: %w", closeErr))
				}
			}
			return nil, openErr
		}
...
func (r *resultDedupChunkReader) nextHeader() (storedResultDedupRecordHeader, bool, error) {
	header, err := readStoredResultDedupRecordHeader(r.reader)
	if errors.Is(err, io.EOF) {
		return storedResultDedupRecordHeader{}, false, nil
	}
	if err != nil {
		return storedResultDedupRecordHeader{}, false, fmt.Errorf("read result deduplication merge input: %w", err)
	}
	return header, true, nil
}

func (r *resultDedupChunkReader) readRecord(header storedResultDedupRecordHeader) (storedResultDedupRecord, error) {
	payload := make([]byte, header.payloadLength)
	if _, err := io.ReadFull(r.reader, payload); err != nil {
		return storedResultDedupRecord{}, fmt.Errorf("read result deduplication merge input: %w", err)
	}
	return storedResultDedupRecord{key: header.key, payload: payload, ordinal: header.ordinal}, nil
}

type resultDedupHeapRecord struct {
	header      storedResultDedupRecordHeader
	readerIndex int
}

type storedResultDedupRecordHeader struct {
	key           string
	ordinal       uint64
	payloadLength int
}
...
func writeStoredResultDedupRecord(writer io.Writer, record storedResultDedupRecord) error {
...
func readStoredResultDedupRecordHeader(reader io.Reader) (storedResultDedupRecordHeader, error) {
	keyLength, err := readResultDedupUint64(reader)
	if err != nil {
		return storedResultDedupRecordHeader{}, err
	}
	...
	if keyLength > uint64(resultDedupMaxInt()) || payloadLength > uint64(resultDedupMaxInt()) {
		return storedResultDedupRecordHeader{}, errors.New("result deduplication record exceeds platform allocation limit")
	}
	...
	return storedResultDedupRecordHeader{key: string(key), ordinal: ordinal, payloadLength: int(payloadLength)}, nil
}
...
// cancellation causes. Joined operational failures remain observable.
