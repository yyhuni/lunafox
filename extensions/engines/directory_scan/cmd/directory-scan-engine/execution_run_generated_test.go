...
func TestGeneratedInputHandlesMaterializeOnlyWhenPathIsCalled(t *testing.T) {
	...
	path, err := input.WebsiteURLs.Path(context.Background())
	if err != nil || path != resolver.paths["websiteURLs"] {
...
func TestGeneratedInputHandlesPropagateContextAndErrors(t *testing.T) {
	...
	if _, err := input.WebsiteURLs.Path(context.Background()); !errors.Is(err, want) {
		t.Fatalf("Path() error = %v, want %v", err, want)
	}
	cancelled, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := input.Subdomains.Path(cancelled); !errors.Is(err, context.Canceled) {
