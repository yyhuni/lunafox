...
function validateManifest(manifest) {
  ...
  if (JSON.stringify(manifest.excludedPaths) !== JSON.stringify([PUBLIC_MANIFEST_PATH])) {
    fail("public export manifest must exclude only its own path");
  }
...
function validatePublicExport(options) {
  ...
  for (const relPath of walk(repoRoot)) {
    ...
    if (relPath !== PUBLIC_MANIFEST_PATH && !manifestPaths.has(relPath)) fail(`public repository path is missing from the exact manifest: ${relPath}`);
  }
