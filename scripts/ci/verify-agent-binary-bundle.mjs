...
function deriveBaseExportManifestSha256(publicExportRoot, tag, sourceRevisionDigest) {
  ...
  const manifest = buildFileManifest(publicExportRoot, { sourceRevisionDigest, releaseTag: tag }, { excludePaths: ["PUBLIC_EXPORT_MANIFEST.json"] });
  const prefix = `agent/bin/${tag}/`;
  manifest.files = manifest.files.filter((file) => !file.path.startsWith(prefix));
  const serialized = `${JSON.stringify({ ...manifest, excludedPaths: ["PUBLIC_EXPORT_MANIFEST.json"] }, null, 2)}\n`;
