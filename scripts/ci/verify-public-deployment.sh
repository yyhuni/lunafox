...
jq -e '
  (.services.agent.stop_grace_period) as $grace |
  ($grace | test("^[0-9]+s$")) and (($grace[0:-1] | tonumber) >= 30)
' <<<"$compose_json" >/dev/null || fail "the resident Agent must allow at least 30 seconds for Engine Container cleanup"
