import type { Vulnerability, GetVulnerabilitiesResponse, VulnerabilitySeverity, VulnerabilityStatsResponse } from '@/types/vulnerability.types'
...
export type VulnerabilityMockStats = VulnerabilityStatsResponse
...
export function getMockVulnerabilities(params?: GetMockVulnerabilitiesParams): GetVulnerabilitiesResponse {
  ...
  if (url !== undefined) {
    filtered = filtered.filter((vulnerability) => vulnerability.url === url)
  }
...
export function bulkDeleteMockVulnerabilities(ids: number[]) {
  const idSet = new Set(ids)
  const originalCount = mockVulnerabilities.length

  for (let index = mockVulnerabilities.length - 1; index >= 0; index -= 1) {
    if (idSet.has(mockVulnerabilities[index].id)) {
      mockVulnerabilities.splice(index, 1)
    }
  }

  return {
    deletedCount: originalCount - mockVulnerabilities.length,
  }
}

export function batchUpdateMockVulnerabilityReviewStates(
  requests: Array<{ name?: string; isReviewed?: boolean }>
) {
  let updatedCount = 0

  for (const request of requests) {
    const id = Number.parseInt((request.name ?? "").replace(/^vulnerabilities\//, ""), 10)
    const vulnerability = mockVulnerabilities.find((item) => item.id === id)
    if (!vulnerability || typeof request.isReviewed !== "boolean") continue
    vulnerability.isReviewed = request.isReviewed
    updatedCount += 1
  }
...
export function updateMockVulnerabilityReviewState(id: number, isReviewed: boolean) {
  const vulnerability = mockVulnerabilities.find((item) => item.id === id)
  if (!vulnerability) return undefined
  vulnerability.isReviewed = isReviewed
...
export function getMockVulnerabilityStats(targetId?: number): VulnerabilityMockStats {
  ...
  const countBySeverity = (severity: VulnerabilitySeverity) =>
    scoped.filter((item) => item.severity === severity).length
