...
const firstVisibleFrameProgress = 0.18;
...
void main() {
    ...
    vec2 p = uv * uScale;
    float signal = getSignal(p);

    if(uChromaticAberration != 0.0){
      vec2 ca = vec2(uChromaticAberration) / iResolution.xy;
      signal = max(signal, max(getSignal(p + ca), getSignal(p - ca)));
    }

    signal = clamp(signal * uBrightness, 0.0, 1.0);
    vec3 col = mix(uBackground, uTint, signal);
...
export default function FaultyTerminal({
  ...
  const minFrameInterval = useMemo(() => (
    typeof maxFps === "number" && Number.isFinite(maxFps) && maxFps > 0
      ? 1000 / maxFps
      : 0
  ), [maxFps]);
  ...
  const tintVec = useMemo(() => {
    void themeRevision;
    return cssColorToRgb(tint);
  }, [tint, themeRevision]);
  const backgroundVec = useMemo(() => {
    void themeRevision;
    return cssColorToRgb(backgroundColor);
  }, [backgroundColor, themeRevision]);
  ...
  useEffect(() => {
    ...
    const update = (t: number) => {
      ...
      if (
        ...
        t - lastRenderTimeRef.current < minFrameInterval
      ) {
        return;
      }
      ...
      let pageLoadProgress = pageLoadAnimation ? program.uniforms.uPageLoadProgress.value : 1;
      if (pageLoadAnimation && loadAnimationStartRef.current > 0) {
      ...
      if (!hasRenderedFirstFrameRef.current && pageLoadProgress >= firstVisibleFrameProgress) {
