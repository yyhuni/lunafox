...
describe("add-target-dialog-sections contract", () => {
  ...
  it("adapts target creation to the shared multi-select organization workspace", () => {
    ...
    expect(source).toContain('Collapsible defaultOpen={false} className="border-t border-border/60 pt-3"')
    expect(source).toContain('CollapsibleContent className="pt-3"')
