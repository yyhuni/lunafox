...
export function AddCustomToolDialog({ tool, onAdd, open: externalOpen, onOpenChange: externalOnOpenChange, }: AddCustomToolDialogProps) {
    const t = useTranslations("tools.config");
    const { open, formData, isEditMode, availableCategories, isFormValid, isSubmitting, handleOpenChange, handleCategoryToggle, handleCategoryRemove, handleSubmit, setFormField, } = useAddCustomToolDialogState({
        tool,
        onAdd,
        open: externalOpen,
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <form onSubmit={handleSubmit}>
          <div className="gap-6 grid py-4">
            ...
            <AddCustomToolCategoriesField t={t} isSubmitting={isSubmitting} availableCategories={availableCategories} selectedCategories={formData.categoryNames} onToggle={handleCategoryToggle} onRemove={handleCategoryRemove}/>
          </div>

          <AddCustomToolFooter t={t} isEditMode={isEditMode} isSubmitting={isSubmitting} isFormValid={isFormValid}/>
        </form>
      </DialogContent>
    </Dialog>);
}
