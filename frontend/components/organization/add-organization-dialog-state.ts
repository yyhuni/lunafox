...
export function useAddOrganizationDialogState({
  ...
  const batchCreateTargetMutation = useBatchCreateTargets()
  const [isCreatingTargets, setIsCreatingTargets] = React.useState(false)
  const [targetsExpanded, setTargetsExpanded] = React.useState(false)
  ...
  const targetValidation = React.useMemo(() => {
    ...
    if (lines.length === 0) {
      return { count: 0, invalid: [] as Array<{ index: number; lineNumber: number; originalTarget: string; error: string; type: string }> }
    }
  ...
  const handleToggleTargetsExpanded = React.useCallback(() => {
    setTargetsExpanded((expanded) => !expanded)
  }, [])
        ...
        onSuccess: async (newOrganization) => {
          if (values.targets && values.targets.trim()) {
            ...
            if (targetList.length > 0) {
              ...
              try {
                await batchCreateTargetMutation.mutateAsync({
                  targets: targetList,
  ...
  }, [batchCreateTargetMutation, createOrganization, form, onAdd, setOpen, targetValidation.invalid.length])
