...
`PageHeader` owns the shared title, code, action, and description rhythm for route-level pages. Its optional `middle` slot creates the wide-screen three-column command layout used by `/overview/`: the title/code is anchored at the left-bottom of the command row, while the secondary middle context and commands share the row's bottom edge and the middle context remains horizontally centered. The display page-title role provides the 24px heading scale. Do not offset a page-local heading or action with custom margins to compensate for this layout.
