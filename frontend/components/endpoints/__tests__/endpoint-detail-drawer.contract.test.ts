...
const drawerPath = path.resolve(process.cwd(), "components/endpoints/endpoint-detail-drawer.tsx")
const source = existsSync(drawerPath) ? readFileSync(drawerPath, "utf8") : ""

describe("endpoint-detail-drawer contract", () => {
  it("uses the shared read-only detail drawer with a single inspection page", () => {
    ...
    expect(source).toContain("EndpointDrawerSection")
    expect(source).toContain('label={tColumns("endpoint.technologies")}')
    expect(source).toContain('label={tColumns("endpoint.responseHeaders")}')
    expect(source).toContain('label={tColumns("endpoint.responseBody")}')
  })
