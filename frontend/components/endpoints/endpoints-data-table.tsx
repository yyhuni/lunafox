...
export function EndpointsDataTable({
  ...
  const endpointFacetPanelItems: DataTableFacetPanelFacet[] = [
    {
      id: "statusCode",
  ...
  const toolbarLeft = (
    <div className="flex w-full min-w-0 flex-wrap items-center gap-2">
      ...
      <DataTableFacetPanel
        ...
        facets={endpointFacetPanelItems}
      />
    </div>
  )
    ...
    <BusinessListDataTable
      ...
      behavior={{
        onRowClick: onRowClick
          ? (row) => onRowClick(row as Endpoint)
          : undefined,
        getRowActionLabel: (row) => `${tActions("details")}: ${(row as Endpoint).url}`,
      }}
      ui={{
        toolbarLeft,
