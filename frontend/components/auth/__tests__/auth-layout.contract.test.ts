...
describe("auth-layout contract", () => {
  ...
  it("uses a desktop left-right shell with the sidebar before the top bar", () => {
    ...
    expect(protectedAuthLayoutSource).toContain('import { ScrollArea } from "@/components/ui/scroll-area"')
    expect(protectedAuthLayoutSource).toContain('protectedAppShellContentFrameClassName,')
    expect(protectedAuthLayoutSource).toContain('protectedAppShellContentStyle,')
    expect(protectedAuthLayoutSource).toContain('protectedAppShellScrollAreaClassName,')
    expect(protectedAuthLayoutSource).toContain('protectedAppShellScrollViewportClassName,')
    ...
    expect(protectedAuthLayoutSource).toContain("contentClassName={protectedAppShellContentFrameClassName}")
    expect(protectedAuthLayoutSource).toContain("contentStyle={protectedAppShellContentStyle}")
    expect(protectedAuthLayoutSource).toContain('type="always"')
    expect(protectedAuthLayoutSource).toContain("viewportClassName={protectedAppShellScrollViewportClassName}")
    ...
    expect(protectedShellSource).toContain('"--header-height": "calc(var(--spacing) * 9)"')
    expect(protectedShellSource).toContain("export const protectedAppShellScrollAreaClassName")
    expect(protectedShellSource).toContain('"min-h-0 min-w-0 flex-1"')
    expect(protectedShellSource).toContain('protectedAppShellScrollViewportClassName = "!overflow-x-hidden"')
    expect(protectedShellSource).toContain("protectedAppShellContentStyle")
    expect(protectedShellSource).not.toContain("scrollbar-gutter:stable_both-edges")
    expect(protectedShellSource).toContain("export const protectedAppShellContentFrameClassName")
    expect(protectedShellSource).toContain("COMPACT_CONTENT_FRAME_CLASS")
    expect(protectedShellSource).toContain("h-full min-h-0")
