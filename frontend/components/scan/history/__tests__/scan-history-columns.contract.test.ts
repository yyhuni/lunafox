...
describe("scan-history-columns contract", () => {
  ...
  it("wraps complete scan summary and engine Badge groups while preserving engine disclosure", () => {
    ...
    expect(engineColumn).toContain("<TooltipProvider>")
    expect(source).not.toContain("SCAN_HISTORY_ENGINE_TOOLTIP_DELAY_MS")
    expect(source).not.toContain("SCAN_HISTORY_ENGINE_TOOLTIP_CLOSE_DELAY_MS")
  ...
  it("routes status clicks through a page-owned action without bubbling to row click", () => {
    ...
    expect(source).not.toContain("SCAN_HISTORY_STATUS_TOOLTIP_DELAY_MS")
    expect(source).not.toContain("SCAN_HISTORY_STATUS_TOOLTIP_CLOSE_DELAY_MS")
    expect(source).not.toContain("SCAN_HISTORY_TRIGGER_TOOLTIP_DELAY_MS")
    expect(source).not.toContain("SCAN_HISTORY_TRIGGER_TOOLTIP_CLOSE_DELAY_MS")
  })
