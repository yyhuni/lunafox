...
export function buildScanProgressData(scan: ScanRecord): ScanProgressData {
  ...
  if (scan.runtimeTasks) {
    const sortedTasks = [...scan.runtimeTasks]
      .toSorted((a, b) => {
    ...
    for (const task of sortedTasks) {
      const stageName = task.stepId || task.stageId || task.engineId
      const resultCount = task.status === "succeeded"
  ...
  return {
    ...
    plannedEngineIds: scan.plannedEngineIds,
    status: scan.status,
