...
describe("sidebar contract", () => {
  ...
  it("keeps menu rows and content static during hover", () => {
    ...
    expect(source).not.toContain("sidebarMenuContentMotionClassName")
    expect(source).not.toContain("hover:[&>svg:not(.ml-auto)]:translate-x-0.5")
    expect(source).not.toContain("hover:[&>span:not([data-sidebar-navigation-pending])]:translate-x-0.5")
  ...
  it("owns a filled active row for secondary sidebar items while preserving the group rail", () => {
    ...
    expect(subButtonSource).not.toContain("transition-transform")
