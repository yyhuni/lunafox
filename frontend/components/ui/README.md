...
## Overlay Spacing Standards

Ordinary overlay perimeter spacing is 16px horizontally and 12px vertically. `Dialog` and `AlertDialog` share the centered default; `SheetHeader`/`SheetFooter` and `DrawerHeader`/`DrawerFooter` use the same insets. Custom `p-0` partitioned dialogs must apply this rhythm to their header, body perimeter, and footer. Reserve trailing space for the 32px close control in visible title rows. Main groups use 12–16px gaps; avoid restoring local 24px outer padding. Media lightboxes, command pickers, and the internal geometry of diagrams, editors, logs, and empty states retain their domain-owned dimensions. See `openspec/changes/unify-overlay-compact-spacing/inventory.md` for the audited caller dispositions.
...
- The compact header-to-content gap is 12px, represented by the shared `COMPACT_PAGE_RHYTHM_CLASS` (`gap-3 py-3`).
- Production route shells keep the 12px compact gap and shared `px-3` horizontal gutter at every breakpoint. Card internals, complex-content geometry, and shared control hit areas retain their own contracts.
...
- Production pages SHOULD use the compact shared route-shell constants unless a specialized shell owns a documented density or controlled geometry.
...
- Standard desktop geometry uses `min-h-8` with `px-2.5 py-1.5`, `gap-2`, and `rounded-lg`. The menu item remains a dense shell control with a compact 13px/18px label while preserving the shared 32px hit area. Do not add page-local sidebar item height, width, or padding overrides such as `h-10`, `px-3`, or one-off rounded values.
- Task-group headings, including the Workspace group that contains Overview and Search, use the shared non-interactive `SidebarGroupLabel` at `h-7` with `px-3`. `SidebarContent` adds no inter-group gap; `SidebarGroup` owns the `py-1` vertical rhythm. Do not hand-roll sidebar group headings with custom `div` wrappers, custom padding, or button-level hover states.
...
- Readable labels use `textRole.navLabel`, while `SidebarMenuButton` and `SidebarMenuSubButton` own the calmer unselected sidebar foreground, active/hover foreground, and compact `leading-[18px]` clamp. Unselected navigation text uses `text-sidebar-foreground/65`; primary leading icons use `text-sidebar-foreground/55`. Selected and hover states restore the full semantic foreground. Primary items and expanded secondary subitems use the same 32px dense shell and `rounded-lg` selection geometry. Do not replace sidebar labels with page-local `text-xs`, uppercase, custom tracking patches, or route-local color overrides to chase density or contrast.
...
- `ScrollArea` accepts `contentClassName` / `contentStyle` for its intrinsic content boundary and `viewportClassName` for an owning shell's viewport constraint. The protected main workspace uses `type="always"`, a content-level minimum-width override, and a viewport-level horizontal clip so its overlay scrollbar never consumes the physical page gutter; route-level tables and editors retain their own internal overflow owners.
...
- `UnifiedDataTable` owns production business-list tables and uses the dense row rhythm: 40px data rows (`h-10`), dense body cell padding (`px-2 py-0.5`), and a 41px virtual-scroll estimate including the row border.
...
- Cell text uses typography roles such as `tableCellPrimary` or `tableCellSecondary`, which keep dense table body copy at an explicit 18px line-height (`leading-[18px]`) for primary text and 16px for secondary text; do not force text blocks to 32px or 36px.
...
- Dense tabs and pagination-adjacent text should stay explicit too: `textRole.tab` uses an 18px line-height (`leading-[18px]`), `textRole.helperText` uses a 16px line-height (`leading-4`) for truly compact hints, and pagination footer copy uses the compact 12px/16px metadata roles.
...
- Shared `TooltipProvider` owns Tooltip timing: first hover opens after `400ms`, pointer leave closes without an added wait, and adjacent Tooltip switching retains a `400ms` window. Production callers MUST NOT pass `delay`, `closeDelay`, or `timeout` through `TooltipProvider` or `TooltipTrigger`; a genuinely different policy requires an approved shared contract instead of a page-local override.
...
The shared protected application content frame owns the wide-screen width boundary through `COMPACT_CONTENT_FRAME_CLASS`: fluid up to 1624px including the existing two 12px page gutters, leaving 1600px usable content at the default spacing scale. Beyond that limit it centers within the main workspace after the sidebar. Shell chrome and the scroll viewport remain full width. Active page-level canvases may opt out with `data-workspace-width="full"`; see `components/shared/layout/README.md` for caller scope. Route wrappers MUST NOT add a second primary-workspace cap or duplicate gutters. Existing narrower copy, error, reader, editor, and dialog surfaces retain their own constraints.
...
Dialog, Sheet, drawer headers and media previews use `Button` with `size="icon-sm"`, `variant="ghost"` and `overlay-close-control`. This shared treatment owns a 28px hit target (32px on devices with a coarse pointer, including hybrid touch devices), 14px cross, muted foreground, transparent resting background and muted hover surface. Keep localized accessible labels, disabled guards and Button keyboard focus styling. Do not apply this class to filter clearing, item removal or task cancellation.
