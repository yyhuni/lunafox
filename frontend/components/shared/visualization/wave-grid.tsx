...
interface WaveGridProps {
  ...
  gridGap?: number
  /** Color of the squares as a theme token or CSS color */
  color?: string
...
export function WaveGrid({
  ...
  const fillColorRef = useRef("currentColor")
  ...
  useEffect(() => {
    ...
    probe.remove()
    fillColorRef.current = resolved || fillColorRef.current
  }, [color])
  ...
  const draw = useCallback(() => {
    ...
    if (waveDirection === "random" && opacitiesRef.current && targetOpacitiesRef.current) {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          ctx.globalAlpha = opacitiesRef.current[index]
    ...
    timeRef.current += 1
    animationRef.current = requestAnimationFrame(draw)
  }, [cols, rows, squareSize, gridGap, width, height, getWaveOpacity, waveDirection, flickerChance, minOpacity, maxOpacity])
