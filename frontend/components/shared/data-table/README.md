...
- Absolutely positioned virtual rows keep that rhythm only as a minimum height and MUST be measured with their natural content height. Do not apply the fixed `h-10` / `h-16` row class to a virtual row: multiline or expanded cells would otherwise overlap the following row.
...
- `SelectedRowActionBar` owns the final action feedback: default, `success`, and `muted` actions rest at the sidebar's subdued foreground and use its neutral accent surface on hover, while semantic status color stays on the icon. `destructive` actions keep their destructive text and tinted hover background. Its outer shell keeps the established compact 42px height, while visible commands use the shared `selected-row-action-control` footprint (28px on fine pointers, 32px on coarse pointers); its clear-selection affordance uses the matching `selection-clear-control` footprint while retaining the action-bar hover treatment; it is not an overlay close control. Call sites MUST NOT add page-local hover classes or infer a persistently emphasized action from array order.
