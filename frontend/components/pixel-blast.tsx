...
function resolveCssColor(color: string) {
  if (typeof document === 'undefined' || !document.body) return new THREE.Color(1, 1, 1);
  ...
  canvas.height = 1;
  const ctx = canvas.getContext('2d');
  if (!ctx) return new THREE.Color(1, 1, 1);
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      t.uniforms.uColor.value.copy(resolvedColor);
