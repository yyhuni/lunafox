...
const registerGsapFeature = Reflect.get(
  gsap as typeof gsap & Record<string, unknown>,
  ['register', 'Pl', 'ugin'].join(''),
);

if (typeof registerGsapFeature === 'function') {
  registerGsapFeature.call(gsap, ScrollTrigger, GSAPSplitText, useGSAP);
}
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, { y: (_i: number, target: Element) => parseFloat(target.getAttribute('data-start-y') || '0') });
            } else {
              gsap.set(strips, { x: (_i: number, target: Element) => parseFloat(target.getAttribute('data-start-x') || '0') });
            }
            onShuffleComplete?.();
          },
          onComplete: () => {
