...
type AipPaginatedResponse<T> = PaginatedResponse<T> & {
  totalSize?: number
  nextPageToken?: string
}

function resolveSubdomainDisplayName(item: { name?: string; dnsName?: string }) {
  return item.dnsName || item.name || ""
}

function normalizePaginatedSubdomainResponse<T extends { name: string; dnsName?: string }>(
  response: AipPaginatedResponse<T>,
  fallbackPage: number,
  ...
  return {
    ...
    results: response.results.map((item) => {
      const displayName = resolveSubdomainDisplayName(item)
      return {
        ...item,
        name: displayName,
        dnsName: displayName,
...
export class SubdomainService {
   ...
   * Backend contract: max 5,000 subdomains per request.
   */
  static async bulkCreateSubdomains(
    ...
    const response = await api.post<BulkCreateSubdomainsResponse>(
      `/targets/${targetId}/subdomains:batchCreate`,
      { dnsNames: subdomains }
    )
  ...
  static async bulkDeleteSubdomains(
    ...
    const response = await api.post<{
      ...
      `/subdomains:batchDelete`,
      { names: ids.map((id) => `targets/${targetId}/subdomains/${id}`) }
    )
    return response.data
  }
  ...
  static async getSubdomainsByTargetId(
    ...
    const response = await api.get<AipPaginatedResponse<BasicSubdomainResult>>(`/targets/${targetId}/subdomains/`, {
      params: {
        pageSize,
    ...
    return normalizePaginatedSubdomainResponse(response.data, 1, pageSize)
  }
  ...
  static async getSubdomainsByScanId(
    ...
    const pageSize = params?.pageSize || 10
    const response = await api.get<AipPaginatedResponse<ScanSubdomainResult>>(`/scans/${scanId}/subdomains/`, {
      params: {
        pageSize,
        ...(params?.pageToken && { pageToken: params.pageToken }),
        ...(params?.filter && { filter: params.filter }),
  ...
  static async exportSubdomainsByTargetId(targetId: number): Promise<Blob> {
    const response = await api.get<Blob>(`/targets/${targetId}/subdomains/exportFiles/current`, {
      responseType: 'blob',
    })
    return response.data
  }
  ...
  static async exportSubdomainsByScanId(scanId: number): Promise<Blob> {
    const response = await api.get<Blob>(`/scans/${scanId}/subdomains/exportFiles/current`, {
      responseType: 'blob',
    })
    return response.data
  }
