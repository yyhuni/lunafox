...
export function useBatchDeleteSubdomains() {
  return useResourceMutation({
    mutationFn: (data: { targetId: number; ids: number[] }) =>
      SubdomainService.bulkDeleteSubdomains(data.targetId, data.ids),
    loadingToast: {
...
export function useScanSubdomains(
  ...
  return useQuery({
    queryKey: ['scans', scanId, 'subdomains', {
      pageSize: params?.pageSize,
      pageToken: params?.pageToken,
      filter: params?.filter,
      orderBy: params?.orderBy,
...
export function useExportSubdomains({
  targetId,
  scanId,
}: {
  targetId?: number
  ...
  return useCallback(() => {
    if (scanId) {
      return SubdomainService.exportSubdomainsByScanId(scanId)
    }
    if (targetId) {
      return SubdomainService.exportSubdomainsByTargetId(targetId)
    }
