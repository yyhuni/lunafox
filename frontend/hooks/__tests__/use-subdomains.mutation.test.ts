...
describe("use-subdomains mutation", () => {
  ...
  it("批量创建子域名含跳过项时走 partialSuccess 并保持失效范围", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("bulk-create-subdomains")
  ...
  it("删除子域名失败时保留错误码映射与 fallback key", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("delete-subdomain-12")
  ...
  it("批量删除子域名成功时保留 bulkSuccess 提示与失效范围", async () => {
    subdomainServiceMocks.bulkDeleteSubdomains.mockResolvedValue({
      message: "ok",
      deletedCount: 2,
    })
    ...
    await act(async () => {
      await result.current.mutateAsync({ targetId: 7, ids: [1, 2] })
    })

    expect(subdomainServiceMocks.bulkDeleteSubdomains).toHaveBeenCalledWith(7, [1, 2])
    expect(toastMocks.loading).toHaveBeenCalledWith(
      "common.status.batchDeleting",
      {},
      "batch-delete-subdomains"
    )
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("batch-delete-subdomains")
    expect(toastMocks.success).toHaveBeenCalledWith(
