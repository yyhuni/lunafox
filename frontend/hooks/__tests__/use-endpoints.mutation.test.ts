...
describe("use-endpoints mutation", () => {
  ...
  it("创建 endpoint 出现已存在项时走 partialSuccess 且失效 endpoints", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("create-endpoint")
  ...
  it("批量创建 endpoint createdCount 为 0 时走 partialSuccess 警告路径", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("bulk-create-endpoints")
    expect(toastMocks.warning).toHaveBeenCalledWith(
      "toast.asset.endpoint.create.partialSuccess",
      { success: 0, skipped: 0 },
      "bulk-create-endpoints"
  ...
  it("删除 endpoint 失败时保留错误码映射与 fallback key", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("delete-endpoint-55")
  ...
  it("删除 endpoint 成功时保留 success 提示与失效范围", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("delete-endpoint-55")
    expect(toastMocks.success).toHaveBeenCalledWith(
      "toast.asset.endpoint.delete.success",
      undefined,
      "delete-endpoint-55"
    )
    expect(invalidateSpy).toHaveBeenCalledWith({ queryKey: endpointKeys.all })
  ...
  it("批量删除 endpoint 成功时保留 bulkSuccess 提示与失效范围", async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ targetId: 9, ids: [1, 2] })
    })
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("batch-delete-endpoints")
    expect(toastMocks.success).toHaveBeenCalledWith(
  ...
  it("批量删除 endpoint 失败时保留错误码映射与 fallback key", async () => {
    ...
    await act(async () => {
      await expect(
        result.current.mutateAsync({ targetId: 9, ids: [1] })
      ).rejects.toBeDefined()
    })
