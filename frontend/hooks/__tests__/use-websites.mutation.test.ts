...
describe("use-websites mutation", () => {
  ...
  it("批量创建网站成功时保留成功提示与失效范围", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("bulk-create-websites")
    expect(toastMocks.success).toHaveBeenCalledWith(
      "toast.asset.website.create.success",
      { count: 2 },
      "bulk-create-websites"
    )
  ...
  it("批量删除网站成功时保留 bulkSuccess 提示与失效范围", async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ targetId: 7, ids: [11, 12] })
    })
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("bulk-delete-websites")
    expect(toastMocks.success).toHaveBeenCalledWith(
  ...
  it("批量删除网站失败时保留错误码映射与 fallback key", async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync({ targetId: 7, ids: [11] })).rejects.toBeDefined()
    })

    expect(websiteServiceMocks.bulkDeleteWebSites).toHaveBeenCalled()
    expect(websiteServiceMocks.bulkDeleteWebSites.mock.calls[0]).toEqual([7, [11]])
    expect(toastMocks.loading).toHaveBeenCalledWith(
      "common.status.batchDeleting",
      {},
      "bulk-delete-websites"
  ...
  it("删除网站成功时保留 success 提示与失效范围", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("delete-website-11")
    expect(toastMocks.success).toHaveBeenCalledWith(
      "toast.asset.website.delete.success",
      undefined,
      "delete-website-11"
    )
