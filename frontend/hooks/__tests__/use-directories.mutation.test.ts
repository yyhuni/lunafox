...
describe("use-directories mutation", () => {
  ...
  it("批量创建目录成功时保留成功提示与目录列表失效范围", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("bulk-create-directories")
    expect(toastMocks.success).toHaveBeenCalledWith(
      "toast.asset.directory.create.success",
      { count: 3 },
      "bulk-create-directories"
    )
  ...
  it("批量删除目录成功时保留 bulkSuccess 提示与失效范围", async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ targetId: 8, ids: [31, 32] })
    })
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("bulk-delete-directories")
    expect(toastMocks.success).toHaveBeenCalledWith(
  ...
  it("批量删除目录失败时保留错误码映射与 fallback key", async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync({ targetId: 8, ids: [31] })).rejects.toBeDefined()
    })

    expect(directoryServiceMocks.bulkDeleteDirectories).toHaveBeenCalled()
    expect(directoryServiceMocks.bulkDeleteDirectories.mock.calls[0]).toEqual([8, [31]])
    expect(toastMocks.loading).toHaveBeenCalledWith(
      "common.status.batchDeleting",
      {},
      "bulk-delete-directories"
  ...
  it("删除目录成功时保留 success 提示与失效范围", async () => {
    ...
    expect(toastMocks.dismiss).not.toHaveBeenCalledWith("delete-directory-31")
    expect(toastMocks.success).toHaveBeenCalledWith(
