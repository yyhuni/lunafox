...
export class TargetValidator {
   ...
   */
  static parseLines(input: string): ParsedTargetLine[] {
   ...
   */
  static validateTargetBatch(
    targets: Array<string | ParsedTargetLine>
  ): Array<TargetValidationResult & { index: number; lineNumber: number; originalTarget: string }> {
    return targets.map((item, index) => {
      const target = typeof item === 'string' ? item : item.target
      const lineNumber = typeof item === 'string' ? index + 1 : item.lineNumber
