...
/**
 * Workflow capability configuration (using global CSS colors)
 * Used for scan initiation, quick scan and other workflow selection interfaces
 */
...
/**
 * Get main icon based on workflow capabilities
...
/**
 * Parse workflow configuration to get capability list
 * Only matches top-level YAML keys (not comments or nested content)
 */
export function parseWorkflowCapabilities(configuration: string): string[] {
...
/**
 * Merge multiple workflow configurations into a single YAML string
 * Simply concatenates configurations with separators
 */
export function mergeWorkflowConfigurations(configurations: string[]): string {
