...
- 安装器会对 `AGENT_IMAGE_REFS` / `WORKER_IMAGE_REFS` 的每个 digest ref 先执行 `docker buildx imagetools inspect` 探测。
- 排序规则固定为：探测成功优先、成功内按耗时升序、同耗时按输入顺序、探测失败放末尾。
- 即使探测失败，仍会作为回退候选继续尝试 `docker pull`，避免因瞬时探测失败导致安装中断。
