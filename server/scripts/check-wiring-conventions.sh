...
module_prefix() {
	...
	scan) echo "Scan" ;;
	taskprogresslog) echo "TaskProgressLog" ;;
	security) echo "Security" ;;
	snapshot) echo "Snapshot" ;;
	*)
		local first="${module:0:1}"
		local rest="${module:1}"
		echo "$(printf '%s' "$first" | tr '[:lower:]' '[:upper:]')${rest}"
		;;
	esac
}
...
for module_dir in "${MODULE_DIRS[@]}"; do
	...
	direct_service_return="$(
		rg -n --no-heading 'return[[:space:]]+[A-Za-z0-9_]+\.[A-Za-z0-9_]*New[A-Za-z0-9]+Service\(' "$exports_file" |
			rg -v 'New[A-Za-z0-9]+ApplicationService\(' || true
	)"
	if [[ -n "$direct_service_return" ]]; then
		append_violation "wiring/$module exports.go 不得直接 return 模块内部 New*Service；请改为模块级 New*ApplicationService" "$direct_service_return"
	fi
done
