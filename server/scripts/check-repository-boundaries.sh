...
if [[ ${#QUERY_FILES[@]} -gt 0 ]]; then
	output="$(rg -n --no-heading -e 'func[[:space:]]+\(.*\)[[:space:]]+(Create|Update|Delete|Batch[A-Za-z0-9_]*|SoftDelete|Mark[A-Za-z0-9_]*|Save|Cancel[A-Za-z0-9_]*|Fail[A-Za-z0-9_]*|Unlock[A-Za-z0-9_]*|Unlink[A-Za-z0-9_]*|Add[A-Za-z0-9_]*)\(' "${QUERY_FILES[@]}" || true)"
	if [[ -n "$output" ]]; then
