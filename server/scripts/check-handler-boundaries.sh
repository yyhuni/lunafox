...
# Rule: handler must not depend on concrete application *Service types directly.
concrete_service_dependencies="$(
	rg -n --no-heading '\*[A-Za-z0-9_]+\.[A-Z][A-Za-z0-9]*Service\b' "${HANDLER_FILES[@]}" || true
)"
if [[ -n "$concrete_service_dependencies" ]]; then
	append_violation "禁止 handler 直接依赖 concrete application *Service；请改为 facade 或显式接口" "$concrete_service_dependencies"
fi

# Rule: handler must not depend on domain repository interfaces directly.
domain_repository_dependencies="$(
	rg -n --no-heading '\b[A-Za-z0-9_]+domain\.[A-Z][A-Za-z0-9]*Repository\b' "${HANDLER_FILES[@]}" || true
)"
if [[ -n "$domain_repository_dependencies" ]]; then
	append_violation "禁止 handler 直接依赖 domain repository 接口；请改为 facade 或显式 application lookup/interface" "$domain_repository_dependencies"
fi
