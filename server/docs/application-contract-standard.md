...
- 新代码不再使用 `contracts.go`；包含端口的文件统一使用资源化 `*_ports.go`，仅承载错误/类型别名的文件按职责拆分为资源化 `*_errors.go`、`*_models.go` 等。
- 禁止使用泛名文件：`ports.go`、`commands.go`、`service.go`、`types_alias.go`。
- 端口文件名应体现资源与职责，例如：`scan_create_target_lookup_ports.go`、`task_execution_query_ports.go`。
