...
func TestAgentAuthMiddleware_StoresAgentIdentityViaAccessors(t *testing.T) {
	...
	router := gin.New()
	router.Use(AgentAuthMiddleware(&agentFinderStub{agent: &agentdomain.Agent{ID: 7, DisplayName: "agent-7"}}))
	router.GET("/protected", func(c *gin.Context) {
		...
		c.JSON(http.StatusOK, gin.H{"agentId": agentID, "name": agent.DisplayName})
	})
