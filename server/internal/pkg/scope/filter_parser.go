...
func parseFilter(filterStr string) []FilterGroup {
	...
	for _, token := range tokens {
		...
		case "__AND__":
			pendingOp = LogicalAnd
		default:
			idx, ok := parseFilterPlaceholder(trimFilterPlaceholderToken(token))
			...
			groups = append(groups, FilterGroup{
				Filter: ParsedFilter{
					Field:    match[1],
					Operator: match[2],
					Value:    unescapeFilterValue(match[3]),
				},
				LogicalOp: logicalOp,
			})
			pendingOp = LogicalAnd
...
func trimFilterPlaceholderToken(token string) string {
	return strings.Trim(strings.TrimSpace(token), "()")
}

func unescapeFilterValue(value string) string {
	quoted := `"` + value + `"`
