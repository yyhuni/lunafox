...
type Scan struct {
	...
	ScanWorkflowID          string         `gorm:"column:scan_workflow_id;type:varchar(100);not null" json:"scanWorkflowId"`
	Configuration           datatypes.JSON `gorm:"column:configuration;type:jsonb" json:"configuration"`
	InputSource             string         `gorm:"column:input_source;size:32;not null" json:"inputSource"`
	TriggerType             string         `gorm:"column:trigger_type;size:16;not null" json:"triggerType"`
	...
	AgentStatus             string         `gorm:"column:agent_status;->" json:"agentStatus"`
	AgentHealthState        string         `gorm:"column:agent_health_state;->" json:"agentHealthState"`
	AgentDeleted            bool           `gorm:"column:agent_deleted;->" json:"agentDeleted"`
	AssignmentMode          string         `gorm:"column:assignment_mode;size:20;not null;default:'automatic'" json:"assignmentMode"`
	...
	// These audit fields are written only by lifecycle SQL updates. Keeping the
	// model projection read-only prevents ordinary Scan creation from requiring
	// columns that are absent in disposable test fixtures while preserving the
	// production baseline columns for upgrade cancellation reads.
