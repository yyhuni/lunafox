...
func TestScanTransition(t *testing.T) {
	now := time.Now()
	scan := &Scan{TargetID: 1, Status: ScanStatusPending, CreatedAt: now}
	...
	if scan.Status != ScanStatusSucceeded {
		t.Fatalf("expected succeeded, got %s", scan.Status)
	}
...
func TestScanFailValidation(t *testing.T) {
	...
	if err := scan.MarkFailed("engine timeout", time.Now()); err != nil {
...
func TestResolveScanStatusFromTaskCounts(t *testing.T) {
	status, shouldUpdate := ResolveScanStatusFromTaskCounts(1, 0, 0, 0, 0)
	if shouldUpdate {
		t.Fatalf("pending tasks should not trigger terminal status, got %s", status)
	}

	status, shouldUpdate = ResolveScanStatusFromTaskCounts(0, 0, 2, 0, 1)
	if !shouldUpdate || status != ScanStatusFailed {
		t.Fatalf("expected failed terminal status, got %s update=%v", status, shouldUpdate)
	}

	status, shouldUpdate = ResolveScanStatusFromTaskCounts(0, 0, 0, 1, 1)
	if !shouldUpdate || status != ScanStatusCancelled {
		t.Fatalf("expected cancelled terminal status, got %s update=%v", status, shouldUpdate)
	}

	status, shouldUpdate = ResolveScanStatusFromTaskCounts(0, 0, 0, 0, 1)
	if !shouldUpdate || status != ScanStatusSucceeded {
		t.Fatalf("expected skipped-only work to resolve as succeeded, got %s update=%v", status, shouldUpdate)
	}

	status, shouldUpdate = ResolveScanStatusFromTaskCounts(0, 0, 0, 0, 0)
	if !shouldUpdate || status != ScanStatusSucceeded {
		t.Fatalf("expected succeeded terminal status, got %s update=%v", status, shouldUpdate)
	}
}
