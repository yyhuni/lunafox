...
type MockVulnerabilitySnapshotService struct {
	SaveAndSyncFunc     func(scanID int, items []dto.VulnerabilitySnapshotItem) (service.MaterializationSummary, error)
	ListByScanFunc      func(scanID int, query *dto.VulnerabilitySnapshotListQuery) ([]service.VulnerabilitySnapshot, int64, error)
	ListAcrossScansFunc func(query *dto.VulnerabilitySnapshotListQuery) ([]service.VulnerabilitySnapshot, int64, error)
	GetByIDFunc         func(id int) (*service.VulnerabilitySnapshot, error)
	CountByScanFunc     func(scanID int) (int64, error)
}

func (m *MockVulnerabilitySnapshotService) SaveAndSync(scanID int, items []dto.VulnerabilitySnapshotItem) (service.MaterializationSummary, error) {
	if m.SaveAndSyncFunc != nil {
		return m.SaveAndSyncFunc(scanID, items)
	}
	return service.MaterializationSummary{}, nil
}
...
func (m *MockVulnerabilitySnapshotService) ListAcrossScans(query *dto.VulnerabilitySnapshotListQuery) ([]service.VulnerabilitySnapshot, int64, error) {
	if m.ListAcrossScansFunc != nil {
		return m.ListAcrossScansFunc(query)
	}
	return nil, 0, nil
}
...
// TestVulnerabilitySnapshotBatchCreate tests the BatchCreate endpoint
func TestVulnerabilitySnapshotBatchCreate(t *testing.T) {
	...
	tests := []struct {
		...
		scanID         string
		body           string
		mockFunc       func(scanID int, items []dto.VulnerabilitySnapshotItem) (service.MaterializationSummary, error)
		expectedStatus int
		expectedBody   string
	}{
		{
			...
			mockFunc: func(scanID int, items []dto.VulnerabilitySnapshotItem) (service.MaterializationSummary, error) {
				return service.MaterializationSummary{SnapshotCount: 1, AssetCount: 1}, nil
			},
		...
		},
		{
			...
			body:   `{"vulnerabilities":[{"url":"https://example.com/vuln","vulnType":"XSS","severity":"high"}]}`,
			mockFunc: func(scanID int, items []dto.VulnerabilitySnapshotItem) (service.MaterializationSummary, error) {
				return service.MaterializationSummary{}, service.ErrScanNotFoundForSnapshot
			},
		...
		},
		{
			...
			mockFunc: func(scanID int, items []dto.VulnerabilitySnapshotItem) (service.MaterializationSummary, error) {
				return service.MaterializationSummary{SnapshotCount: 2, AssetCount: 2}, nil
			},
			expectedStatus: http.StatusOK,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := gin.New()
			router.POST("/v1/scans/:scan/vulnerabilities:batchCreate", func(c *gin.Context) {
				scanID := c.Param("scan")
				if scanID == "invalid" {
				...
				var req dto.BatchCreateVulnerabilitySnapshotsRequest
				if err := c.ShouldBindJSON(&req); err != nil {
					httpdto.BadRequest(c, "Invalid request body")
					return
				}

				summary, err := mockSvc.SaveAndSync(1, req.Vulnerabilities)
				if err != nil {
					if err == service.ErrScanNotFoundForSnapshot {
						httpdto.NotFound(c, "Scan not found")
						return
					}
					httpdto.InternalError(c, "Failed to save vulnerability snapshots")
					return
				}

				httpdto.Success(c, dto.BatchCreateVulnerabilitySnapshotsResponse{
					SnapshotCount: int(summary.SnapshotCount),
					AssetCount:    int(summary.AssetCount),
				})
			})

			req := httptest.NewRequest(http.MethodPost, "/v1/scans/"+tt.scanID+"/vulnerabilities:batchCreate", strings.NewReader(tt.body))
...
func TestVulnerabilitySnapshotListByScan(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedStatus: http.StatusOK,
			checkResponse: func(t *testing.T, body string) {
				var resp httpdto.PaginatedResponse[dto.VulnerabilitySnapshotResponse]
				...
				if resp.NextPageToken != "" {
					t.Errorf("expected no nextPageToken, got %q", resp.NextPageToken)
				}
			},
		},
		{
			...
			scanID:      "1",
			queryParams: "?pageToken=cGFnZToy&pageSize=10",
			mockFunc: func(scanID int, query *dto.VulnerabilitySnapshotListQuery) ([]service.VulnerabilitySnapshot, int64, error) {
				if query.GetPage() != 2 {
			...
			checkResponse: func(t *testing.T, body string) {
				...
				if resp.TotalSize != 30 {
					t.Errorf("expected totalSize 30, got %d", resp.TotalSize)
				}
				if resp.NextPageToken == "" {
					t.Error("expected nextPageToken for additional results")
				}
			},
		},
		{
			...
			scanID:      "1",
			queryParams: "?filter=severity%3D%22critical%22",
			mockFunc: func(scanID int, query *dto.VulnerabilitySnapshotListQuery) ([]service.VulnerabilitySnapshot, int64, error) {
				if query.Filter != `severity="critical"` {
					t.Errorf("expected severity filter, got %q", query.Filter)
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := gin.New()
			router.GET("/v1/scans/:scan/vulnerabilities/", func(c *gin.Context) {
				scanID := c.Param("scan")
				if scanID == "invalid" {
					httpdto.BadRequest(c, "Invalid scan ID")
					return
				...
				if err := c.ShouldBindQuery(&query); err != nil {
					httpdto.BadRequest(c, "Invalid query parameters")
					return
				}
				...
				if err != nil {
					...
					httpdto.InternalError(c, "Failed to list vulnerability snapshots")
					return
				}
			...
			req := httptest.NewRequest(http.MethodGet, "/v1/scans/"+tt.scanID+"/vulnerabilities/"+tt.queryParams, nil)
			w := httptest.NewRecorder()
...
// TestVulnerabilitySnapshotListAcrossScans tests the cross-scan list endpoint.
func TestVulnerabilitySnapshotListAcrossScans(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mockSvc := &MockVulnerabilitySnapshotService{
				ListAcrossScansFunc: tt.mockFunc,
			}

			router := gin.New()
			router.GET("/v1/vulnerabilitySnapshots/", func(c *gin.Context) {
				var query dto.VulnerabilitySnapshotListQuery
				if err := c.ShouldBindQuery(&query); err != nil {
					httpdto.BadRequest(c, "Invalid query parameters")
					return
				...
				snapshots, total, err := mockSvc.ListAcrossScans(&query)
				if err != nil {
					httpdto.InternalError(c, "Failed to list vulnerability snapshots")
					return
				}
			...
			req := httptest.NewRequest(http.MethodGet, "/v1/vulnerabilitySnapshots/"+tt.queryParams, nil)
			w := httptest.NewRecorder()
...
func TestVulnerabilitySnapshotGetByID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router.GET("/v1/vulnerabilitySnapshots/:vulnerability_snapshot/", func(c *gin.Context) {
				...
				if err != nil {
					if err == service.ErrVulnerabilitySnapshotNotFound {
						httpdto.NotFound(c, "Vulnerability snapshot not found")
						return
					}
					httpdto.InternalError(c, "Failed to get vulnerability snapshot")
					return
				}
			...
			req := httptest.NewRequest(http.MethodGet, "/v1/vulnerabilitySnapshots/"+tt.id+"/", nil)
			w := httptest.NewRecorder()
	...
	for _, filter := range filterTests {
		t.Run("filter_"+filter, func(t *testing.T) {
			...
			router.GET("/v1/vulnerabilitySnapshots/", func(c *gin.Context) {
				...
				_, _, _ = mockSvc.ListAcrossScans(&query)
				httpdto.Paginated(c, []dto.VulnerabilitySnapshotResponse{}, 0, 1, 20)
			})

			url := "/v1/vulnerabilitySnapshots/"
			if filter != "" {
				url += "?filter=" + filter
			}
...
func TestVulnerabilitySnapshotSeverityFilterProperties(t *testing.T) {
	...
	for _, severity := range severityTests {
		t.Run("severity_"+severity, func(t *testing.T) {
			...
			url := "/v1/scans/1/vulnerabilities/"
			if severity != "" {
				url += "?filter=severity%3D%22" + severity + "%22"
			}
			req := httptest.NewRequest(http.MethodGet, url, nil)
			w := httptest.NewRecorder()
			...
			expectedFilter := ""
			if severity != "" {
				expectedFilter = `severity="` + severity + `"`
			}
			if receivedFilter != expectedFilter {
				t.Errorf("expected severity filter %q, got %q", expectedFilter, receivedFilter)
			}
		})
	}
}
...
// 2. Batch create vulnerability snapshots
// 3. List snapshots
...
func TestIntegrationCompleteFlow(t *testing.T) {
	...
	t.Run("batch_create", func(t *testing.T) {
		mockSvc := &MockVulnerabilitySnapshotService{
			SaveAndSyncFunc: func(scanID int, items []dto.VulnerabilitySnapshotItem) (service.MaterializationSummary, error) {
				...
				return service.MaterializationSummary{SnapshotCount: count, AssetCount: count}, nil
			},
		}
		...
		req := httptest.NewRequest(http.MethodPost, "/v1/scans/1/vulnerabilities:batchCreate", strings.NewReader(body))
		req.Header.Set("Content-Type", "application/json")
		w := httptest.NewRecorder()
		...
		var resp dto.BatchCreateVulnerabilitySnapshotsResponse
	...
	t.Run("list_by_scan", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/v1/scans/1/vulnerabilities/", nil)
		w := httptest.NewRecorder()
	...
	t.Run("get_by_id", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/v1/vulnerabilitySnapshots/1/", nil)
		w := httptest.NewRecorder()
...
func TestIntegrationConcurrentWrites(t *testing.T) {
	...
	router.POST("/v1/scans/:scan/vulnerabilities:batchCreate", func(c *gin.Context) {
		...
		summary, _ := mockSvc.SaveAndSync(1, req.Vulnerabilities)
		httpdto.Success(c, dto.BatchCreateVulnerabilitySnapshotsResponse{
			SnapshotCount: int(summary.SnapshotCount),
			AssetCount:    int(summary.AssetCount),
		})
...
func TestIntegrationLargeDataset(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/v1/scans/1/vulnerabilities/?pageSize=100", nil)
	w := httptest.NewRecorder()
	...
	if resp.TotalSize != int64(largeCount) {
		t.Errorf("expected totalSize %d, got %d", largeCount, resp.TotalSize)
	}
...
func TestIntegrationFilterAndOrderingCombination(t *testing.T) {
	...
	mockSvc := &MockVulnerabilitySnapshotService{
		ListByScanFunc: func(scanID int, query *dto.VulnerabilitySnapshotListQuery) ([]service.VulnerabilitySnapshot, int64, error) {
			...
			for _, s := range mockSnapshots {
				...
				if strings.Contains(query.Filter, "severity=") && !strings.Contains(query.Filter, s.Severity) {
					continue
				}
				filtered = append(filtered, s)
			}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			req := httptest.NewRequest(http.MethodGet, "/v1/scans/1/vulnerabilities/"+tt.queryParams, nil)
			w := httptest.NewRecorder()
			...
			if int(resp.TotalSize) != tt.expectedCount {
				t.Errorf("expected totalSize %d, got %d", tt.expectedCount, resp.TotalSize)
			}
		})
	}
}
...
func TestIntegrationErrorHandling(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "scan not found on batch create",
			setupMock: func() *MockVulnerabilitySnapshotService {
			...
			path:           "/v1/scans/999/vulnerabilities:batchCreate",
		...
		},
		{
			...
			path:           "/v1/scans/999/vulnerabilities/",
			body:           "",
			expectedStatus: http.StatusNotFound,
		},
		{
			...
			method:         http.MethodGet,
			path:           "/v1/vulnerabilitySnapshots/999/",
			body:           "",
			expectedStatus: http.StatusNotFound,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router.POST("/v1/scans/:scan/vulnerabilities:batchCreate", func(c *gin.Context) {
				...
				_, err := mockSvc.SaveAndSync(999, req.Vulnerabilities)
				if err != nil {
					if err == service.ErrScanNotFoundForSnapshot {
						httpdto.NotFound(c, "Scan not found")
						return
					}
					httpdto.InternalError(c, "Failed to save")
					return
				}
			})
			...
			router.GET("/v1/scans/:scan/vulnerabilities/", func(c *gin.Context) {
				...
				if err != nil {
					...
					httpdto.InternalError(c, "Failed to list")
					return
				}
			})
			...
			router.GET("/v1/vulnerabilitySnapshots/:vulnerability_snapshot/", func(c *gin.Context) {
				...
				if err != nil {
					...
					httpdto.InternalError(c, "Failed to get")
					return
				}
			})
...
type MockWebsiteSnapshotService struct {
	SaveAndSyncFunc  func(scanID int, targetID int, items []dto.WebsiteSnapshotItem) (service.MaterializationSummary, error)
...
func (m *MockWebsiteSnapshotService) SaveAndSync(scanID int, targetID int, items []dto.WebsiteSnapshotItem) (service.MaterializationSummary, error) {
	if m.SaveAndSyncFunc != nil {
...
// TestBatchUpsertHandler tests the BatchUpsert endpoint
func TestBatchUpsertHandler(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	tests := []struct {
		...
		scanID         string
		body           string
		mockFunc       func(scanID int, targetID int, items []dto.WebsiteSnapshotItem) (service.MaterializationSummary, error)
		expectedStatus int
		expectedBody   string
	}{
		{
			name:   "successful batch upsert",
			scanID: "1",
			body:   `{"target":"targets/1","websites":[{"url":"https://example.com","title":"Example"}]}`,
			mockFunc: func(scanID int, targetID int, items []dto.WebsiteSnapshotItem) (service.MaterializationSummary, error) {
				return service.MaterializationSummary{SnapshotCount: 1, AssetCount: 1}, nil
			},
			expectedStatus: http.StatusOK,
		...
		},
		{
			name:           "invalid scan ID",
			scanID:         "invalid",
			body:           `{"target":"targets/1","websites":[]}`,
			expectedStatus: http.StatusBadRequest,
			expectedBody:   `"message":"Invalid scan ID"`,
		},
		{
			name:   "scan not found",
			scanID: "999",
			body:   `{"target":"targets/1","websites":[{"url":"https://example.com"}]}`,
			mockFunc: func(scanID int, targetID int, items []dto.WebsiteSnapshotItem) (service.MaterializationSummary, error) {
				return service.MaterializationSummary{}, service.ErrScanNotFoundForSnapshot
			},
			expectedStatus: http.StatusNotFound,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := gin.New()
			router.POST("/v1/scans/:scanId/websites:batchIngest", func(c *gin.Context) {
				...
				summary, err := mockSvc.SaveAndSync(1, 1, req.Websites)
				if err != nil {
					if err == service.ErrScanNotFoundForSnapshot {
						httpdto.NotFound(c, "Scan not found")
						return
			...
			req := httptest.NewRequest(http.MethodPost, "/v1/scans/"+tt.scanID+"/websites:batchIngest", strings.NewReader(tt.body))
			req.Header.Set("Content-Type", "application/json")
			w := httptest.NewRecorder()
...
func TestListHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedStatus: http.StatusOK,
			checkResponse: func(t *testing.T, body string) {
				var resp httpdto.PaginatedResponse[dto.WebsiteSnapshotResponse]
				if err := json.Unmarshal([]byte(body), &resp); err != nil {
					t.Fatalf("failed to unmarshal response: %v", err)
				}
				if resp.TotalSize != 3 {
					t.Errorf("expected totalSize 3, got %d", resp.TotalSize)
				}
				if resp.NextPageToken != "" {
					t.Errorf("expected no nextPageToken, got %q", resp.NextPageToken)
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := gin.New()
			router.GET("/v1/scans/:scanId/websites", func(c *gin.Context) {
				scanID := c.Param("scanId")
				if scanID == "invalid" {
					httpdto.BadRequest(c, "Invalid scan ID")
					return
			...
			req := httptest.NewRequest(http.MethodGet, "/v1/scans/"+tt.scanID+"/websites"+tt.queryParams, nil)
			w := httptest.NewRecorder()
	...
	for _, filter := range filterTests {
		t.Run("filter_"+filter, func(t *testing.T) {
			...
			router.GET("/v1/scans/:scanId/websites", func(c *gin.Context) {
				...
				_ = c.ShouldBindQuery(&query)
				_, _, _ = mockSvc.ListByScan(1, &query)
				httpdto.Paginated(c, []dto.WebsiteSnapshotResponse{}, 0, 1, 20)
			})

			url := "/v1/scans/1/websites"
			if filter != "" {
				url += "?filter=" + filter
			}
			req := httptest.NewRequest(http.MethodGet, url, nil)
