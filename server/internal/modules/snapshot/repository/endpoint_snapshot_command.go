...
// BatchCreate creates multiple endpoint snapshots, ignoring duplicates
func (r *EndpointSnapshotRepository) BatchCreate(snapshots []snapshotdomain.EndpointSnapshot) (int64, error) {
	return r.BatchCreateContext(context.Background(), snapshots)
}

func (r *EndpointSnapshotRepository) BatchCreateContext(ctx context.Context, snapshots []snapshotdomain.EndpointSnapshot) (int64, error) {
	if len(snapshots) == 0 {
		return 0, nil
	}
	...
	for i := 0; i < len(modelSnapshots); i += batchSize {
		...
		result := dbtx.Resolve(ctx, r.db).WithContext(ctx).Clauses(clause.OnConflict{
			...
			DoUpdates: clause.AssignmentColumns([]string{
				...
				"content_type", "tech", "response_body", "response_body_truncated", "vhost", "response_headers", "response_headers_truncated",
			}),
		}).Create(&batch)
		if result.Error != nil {
