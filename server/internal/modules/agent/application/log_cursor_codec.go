...
func newLogCursorCodec(secret string) (*logCursorCodec, error) {
	key := []byte(strings.TrimSpace(secret))
	if len(key) == 0 {
		return nil, fmt.Errorf("%w: codec key is empty", ErrLogCursorInvalid)
	}
	return &logCursorCodec{key: key}, nil
}
...
func (codec *logCursorCodec) Encode(payload logCursorPayload) (string, error) {
	...
	if normalized.Kind != "follow" && normalized.Kind != "older" {
		return "", fmt.Errorf("%w: unsupported cursor kind", ErrLogCursorInvalid)
	}
...
func (codec *logCursorCodec) Decode(token string) (logCursorPayload, error) {
	...
	if payload.Kind != "follow" && payload.Kind != "older" {
		return logCursorPayload{}, fmt.Errorf("%w: unsupported cursor kind", ErrLogCursorInvalid)
	}
	if payload.LastTsNs == "" ||
		payload.LastID == "" ||
		payload.LastStream == "" ||
