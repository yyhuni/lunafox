...
func TestLocalWordlistFileStoreSavePreservesFileNameWithoutAddingExtension(t *testing.T) {
	store := NewLocalWordlistFileStore()
	basePath := t.TempDir()

	metadata, err := store.Save(basePath, "dictionary", strings.NewReader("one\ntwo\n"))
	if err != nil {
		t.Fatalf("save wordlist without extension failed: %v", err)
	}
	if got, want := filepath.Base(metadata.FilePath), "dictionary"; got != want {
		t.Fatalf("stored basename = %q, want %q", got, want)
	}
	if _, err := os.Stat(filepath.Join(basePath, "dictionary.txt")); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("unexpected extension alias was created: %v", err)
	}
}

func TestLocalWordlistFileStoreSaveRejectsPathLikeFileName(t *testing.T) {
	store := NewLocalWordlistFileStore()
	basePath := t.TempDir()

	_, err := store.Save(basePath, "../dictionary.txt", strings.NewReader("one\n"))
	if !errors.Is(err, catalogdomain.ErrWordlistFileNameInvalid) {
