...
func (store *LocalWordlistFileStore) Save(basePath, filename string, content io.Reader) (*WordlistFileMetadata, error) {
	...
	// The Catalog has already accepted this basename as the immutable fileName.
	// Keep the physical basename byte-for-byte aligned with that value; adding an
	// extension or sanitizing here would make the runtime descriptor ambiguous.
	safeFilename, err := catalogdomain.ValidateWordlistFileName(filename)
	if err != nil {
		return nil, err
	}
	fullPath := filepath.Join(basePath, safeFilename)

	// O_EXCL is the filesystem-side concurrency boundary. A database unique
	// conflict must never be able to overwrite another upload's bytes.
