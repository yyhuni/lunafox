...
func (service *AuthCommandService) Login(ctx context.Context, username, password string) (*LoginResult, error) {
	...
	tokenPair, err := service.tokens.GenerateTokenPair(user.ID, user.Username, user.TokenVersion)
	if err != nil {
		return nil, err
	}
...
func (service *AuthCommandService) RefreshToken(ctx context.Context, refreshToken string) (*RefreshResult, error) {
	...
	user, err := service.users.GetAuthUserByID(claims.UserID)
	if err != nil {
		if dberrors.IsRecordNotFound(err) {
			return nil, ErrInvalidRefreshToken
		}
	...
	accessToken, expiresIn, err := service.tokens.GenerateAccessToken(user.ID, user.Username, user.TokenVersion)
	if err != nil {
		return nil, err
	}
