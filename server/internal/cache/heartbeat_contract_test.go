...
func TestHeartbeatDataDoesNotDefineRuntimeStatusField(t *testing.T) {
	if _, ok := reflect.TypeOf(HeartbeatData{}).FieldByName("RuntimeStatus"); ok {
		t.Fatalf("heartbeat data must not define RuntimeStatus")
	}
}
