...
// HeartbeatData represents the latest heartbeat snapshot cached for an agent.
// CPU/Mem/Disk are stored for monitoring and API exposure only — the server
// does not use them for task scheduling decisions (load gating is agent-side).
type HeartbeatData struct {
	InstanceID string `json:"instanceId"`
	// SessionID is the agent-generated process session identifier reported by
	// the runtime connection that produced this heartbeat.
	SessionID string `json:"sessionId"`
	// SessionEpoch is the server-issued fencing token for the active runtime
	// session when this heartbeat snapshot was stored.
...
func (c *heartbeatCache) Set(ctx context.Context, agentID int, data *HeartbeatData) error {
	if data == nil {
		return fmt.Errorf("heartbeat data is required")
	}
