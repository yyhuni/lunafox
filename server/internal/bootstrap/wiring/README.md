...
- Server-owned execution policies must be injected explicitly in `exports.go`; scan-create v2 planning uses `DefaultPlanTaskLimitsProvider` here and never derives limits from request/config/environment data.
...
- Application service factories: `New<Module>ApplicationService`
  - Example: `NewTaskProgressLogApplicationService`
  - Example: `NewScanApplicationService`
...
- Pattern: `new<Module><Role>Adapter`
  - Example: `newScanTaskRuntimeScanStoreAdapter`
  - Example: `newTaskProgressLogScanLookupAdapter`
...
- `<module>ExecutionStoreAdapter` when the role is actually a concrete resource store, such as a scan task store
...
## Checks
- Wiring convention check script: `bash server/scripts/check-wiring-conventions.sh` (validates exports naming/prefix, adapter returns interface, assertions present).
