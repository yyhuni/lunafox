...
- [x] 2.8 固化“配置仅描述业务参数，不包含版本字段”的 schema 门禁
- [x] 2.9 落地调度兼容门禁：按 workflow 能力判定是否可分配
- [x] 2.10 增加回归用例：workflow 能力不兼容返回 `WORKER_VERSION_INCOMPATIBLE`
- [x] 2.11 增加回归用例：配置结构非法或缺失关键字段返回 `SCHEMA_INVALID`
- [x] 2.12 运行配置与扫描创建相关测试，确认边界行为与文档一致

## 3. 契约与文档链路迁移
- [x] 3.1 设计并实现从 Go 配置定义生成 schema/docs 的自动化代码生成命令
- [x] 3.2 固化产物目录：schema 输出到 `server/internal/workflowschema`，docs 输出到 `docs/config-reference`
- [x] 3.2.1 固化 schema 文件命名规则：`<workflow>.schema.json`
- [x] 3.3 定义 `contracts/gen/workflowschema` 为可选镜像输出（默认关闭，仅对外发布时启用）
...
- [x] 5.2.1 校验生成产物命名符合 workflow 固定命名规则并与镜像输出同名
...
- [x] 5.7 增加端到端用例覆盖 workflow 能力不兼容拦截与错误码映射
- [x] 5.8 增加端到端用例覆盖配置结构非法与 `SCHEMA_INVALID` 映射
