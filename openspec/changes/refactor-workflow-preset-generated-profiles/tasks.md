## 1. 生成链改造（TDD）
- [x] 1.1 为 preset 生成器新增行为测试：`default` 可从契约生成，先 RED
- [x] 1.2 实现生成器最小能力（仅 `default`），并让测试 GREEN
...
## 4. CI 门禁与开发流程
- [x] 4.1 扩展生成脚本，将 preset 生成纳入统一入口
- [x] 4.2 增加 CI 无差异检查，覆盖 preset 目录
- [x] 4.3 更新开发文档：声明 preset 文件为生成产物，不建议手改

## 5. 验证
- [x] 5.1 运行后端相关测试（catalog/workflow/profile/schema/bootstrap）
- [x] 5.2 运行 `openspec validate refactor-workflow-preset-generated-profiles --strict --no-interactive`
